...
for f in os.listdir(dataset_path+'images/malignant/'):
	if '.npy' in f:
		try:
			...
			for e in range(lbl.shape[0]):
				if (np.any(lbl[e]==1).astype(np.int32)!=0): 
					...
					full_annotations      = anchor_targets_bbox(anchors, [img[e].astype(np.float32)], [annotations], 
						                                        ...
						                                        anchor_deltas_mean=anchor_deltas_mean, anchor_deltas_std=anchor_deltas_std)
...
CSVdata_name =  './models/laufey/TumorsdBBOX-32_3S-5R.csv'
